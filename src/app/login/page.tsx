...
function LoginContent() {
  ...
  useEffect(() => {
    if (errorParam === "AccessDenied" || errorParam === "OAuthSignin") {
      setMessage({ 
        text: "Este Gmail não está matriculado. Realize sua 'Nova Matrícula' primeiro para liberar o acesso.", 
        type: "error" 
      });
          ...
          <div className="space-y-6">
            ...
            {isLogin ? (
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex items-center gap-2 text-zinc-100 font-bold text-sm tracking-wide mb-2">
                  <ShieldCheck className="text-primary-500" size={18} fill="currentColor" /> Matrícula Blindada
                </div>
                
                <p className="text-zinc-500 text-xs font-medium leading-relaxed">
                  Para garantir sua vaga e a integridade do sistema, a matrícula é realizada exclusivamente através da sua conta Google (@gmail.com).
                </p>

                <div className="relative group">
                  <div className="absolute inset-0 bg-primary-500/20 blur-xl rounded-2xl opacity-40 group-hover:opacity-60 transition-opacity" />
                  <button 
                    type="button"
                    onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
                    className="relative w-full glass flex items-center justify-center gap-4 py-5 rounded-2xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.08] hover:border-white/20 transition-all active:scale-[0.98]"
                  >
                    ...
                    <span className="text-white font-black text-sm uppercase tracking-[0.2em]">
                      Matricular via Google
                    </span>
                  </button>
                </div>

                <p className="text-[10px] text-zinc-600 leading-relaxed font-black uppercase tracking-widest text-center">
                  Matrícula Instantânea e Segura
                </p>
              </div>
            )}
            ...
            {isLogin && (
              <>
                ...
                {/* Google Login Button - Solo en Login */}
                <button 
                  type="button"
                  onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-zinc-950"><Loader2 className="animate-spin text-primary-500" size={32} /></div>}>
      <LoginContent />
    </Suspense>
  );
}
