...
import { Zap, ShieldCheck, Mail, Lock, Loader2, Info } from "lucide-react";
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2 text-zinc-100 font-bold text-sm tracking-wide">
              {isLogin ? (
                ...
                <><ShieldCheck className="text-emerald-500" size={18} /> Cadastre seu Gmail Seguro</>
              )}
            </div>
            
            <p className="text-zinc-500 text-xs font-medium leading-relaxed mb-6">
              Sua sessão permanecerá criptografada no aparelho. {isLogin ? 'Bem-vindo de volta.' : 'Crie sua senha exclusiva da plataforma.'}
            </p>
            ...
            <form onSubmit={isLogin ? handleLogin : handleRegister} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input 
                    name="password"
                    type="password" 
                    placeholder={isLogin ? "Sua senha..." : "Crie uma senha forte (min. 6 letras)"}
                    required
                    minLength={6}
                    className="w-full bg-black/50 border border-white/5 rounded-2xl py-4 pl-12 pr-4 text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 transition-all font-medium"
                  />
                </div>
                {isLogin && (
              ...
              >
                ...
                <div className="relative flex items-center justify-center gap-3 py-4 bg-zinc-100 text-zinc-950 font-black text-sm uppercase tracking-widest rounded-2xl hover:bg-white active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100">
                  {isLoading ? <Loader2 className="animate-spin" size={18} /> : (isLogin ? "Autorizar Acesso" : "Criar Nova Matrícula")}
                </div>
              </button>
            </form>
          </div>
